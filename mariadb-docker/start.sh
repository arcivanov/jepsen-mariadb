...
export LEIN_OPTIONS="run test --db maria-docker --nodes localhost --concurrency ${NPROC} --rate 1000 --no-ssh=true --innodb-snapshot-isolation --mariadb-install-dir ${MARIADB_DIR} --mariadb-data-dir ${DATA_DIR}"
